...
(defvar gptel-proof-gentle-prompt
  (concat
    "Please fix spelling, punctuation and grammar in the follow text. "
...
(defun gptel-proof-apply-fix (buffer marker correction)
  "Apply the change chatgpt has suggested.
Argument BUFFER the buffer.
Argument MARKER the marker.
Argument CORRECTION correction."
...
(defun gptel-proof (start end &optional aggressive)
  ...
  (let*
    (
      (marker (format "{proof:%s}" (uuid-string)))
